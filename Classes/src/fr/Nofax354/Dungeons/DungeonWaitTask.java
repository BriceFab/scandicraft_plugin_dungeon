	...
	public void run() {
		...
		if(timer == 10 || timer == 5 || timer == 4 || timer == 3 || timer == 2 || timer == 1) {
			for(int i = 0;i < main.dungeonP.size();i++) {
				main.dungeonP.get(i).sendMessage("Lancement du donjon dans "+timer+"s");
			}
		}
		
		if(timer == 0) {
			for(int i = 0;i < main.dungeonP.size();i++) {
				main.dungeonP.get(i).sendMessage("Le jeu va démmarrer");
			}
			
			main.dungeonT.setState(DungeonState.PVP);
			
			for(int i = 0;i < main.dungeon.size();i++) {
				Player p = main.dungeonP.get(i);
				p.getInventory().clear();
				p.setGameMode(GameMode.SURVIVAL);
				p.teleport(new Location(main.dungeonT.getWorld("Donjon"),-0.5D,2.5D,-0.5D));
			}
			main.dungeonXp.put(main.nb, 0);
			main.nb++;
			DungeonTask dungeonTask = new DungeonTask(main,main.nb);
